...
for isub = 1 %:length(animals)
    ...
    for iexper = 1:length(exper_conditions)
        ...
        if strcmp(EXPER_COND,'control')
            colorMap =[70/255 130/255 180/255];
        elseif strcmp(EXPER_COND,'harm')
            colorMap = [178/255 34/255 34/255];
        end
        ...
        for isession = 1:length(ExperSessions)
            ...
            if runSpikeInfo
                ...
                if useJAABA
                    ...
                    for itrial = 1:length(selectIdx) %1:numTrials
                        if isempty(handLift.t0{1,selectIdx(itrial)}{1,1}) == 0
                            startFrames(1,itrial) = handLift.t0{1,selectIdx(itrial)}{1,1}(1);
                        else
                ...
                sgtitle([SUB ' ' EXPER_SESSION ': ' EXPER_COND ' session firing rates']);
                ...
                ylabel('PC 2');
                zlabel('PC 3');
                title([SUB ' ' EXPER_SESSION ': ' EXPER_COND ' session']);
                s=diag(s);
                VAR_total=sum(s.^2);
                VAF=s.^2/VAR_total;
                %% ISI
            ...
            if runLFPInfo
                ...
                rec_time= length(data(chan2Plot,:))./sampling_frequency; % total recording time
                ...
                plot(freq_axis,Pow_spec2,'Color',colorMap); % Plot power spectrum
